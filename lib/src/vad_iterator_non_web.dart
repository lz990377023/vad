...
class VadIteratorNonWeb implements VadIteratorBase {
  ...
  var _state = List.filled(2, List.filled(_batch, Float32List.fromList(List.filled(128, 0.0))));
  ...
  void reset() {
    ...
    _state = List.filled(2, List.filled(_batch, Float32List.fromList(List.filled(128, 0.0))));
  }
  ...
  Future<(double, List<OrtValue?>)> _runV5ModelInference(Float32List data) async {
  ...
  Future<(double, List<OrtValue?>)> _runLegacyModelInference(Float32List data) async {
    final inputOrt = OrtValueTensor.createTensorWithDataList(data, [_batch, frameSamples]);
    final srOrt = OrtValueTensor.createTensorWithData(sampleRate);
  ...
  Uint8List _combineSpeechBuffer() {
    ...
    final int16Data =
        Int16List.fromList(combined.map((e) => (e * 32767).clamp(-32768, 32767).toInt()).toList());
