...
class VadHandlerWeb implements VadHandlerBase {
  ...
  final StreamController<({double isSpeech, double notSpeech, List<double> frame})>
  ...
  final StreamController<Stream<Uint8List>> _onAudioStreamController =
      StreamController<Stream<Uint8List>>.broadcast();
  ...
  @override
  Stream<Stream<Uint8List>> get onAudioStream => _onAudioStreamController.stream;
  ...
  void startListening(
    ...
    startListeningImpl(positiveSpeechThreshold, negativeSpeechThreshold, preSpeechPadFrames, redemptionFrames,
        frameSamples, minSpeechFrames, submitUserSpeechOnPause, model, baseAssetPath, onnxWASMBasePath);
  }
        ...
        case 'onSpeechEnd':
          if (eventData.containsKey('audioData')) {
            final List<double> audioData =
                (eventData['audioData'] as List).map((e) => (e as num).toDouble()).toList();
            if (isDebug) {
        ...
        case 'onFrameProcessed':
          if (eventData.containsKey('probabilities') && eventData.containsKey('frame')) {
            ...
            final List<double> frame =
                (eventData['frame'] as List).map((e) => (e as num).toDouble()).toList();

            if (isDebug) {
              debugPrint('VadHandlerWeb: onFrameProcessed: isSpeech: $isSpeech, notSpeech: $notSpeech');
            }

            _onFrameProcessedController.add((isSpeech: isSpeech, notSpeech: notSpeech, frame: frame));
          } else {
            if (isDebug) {
  ...
  void dispose() {
    ...
    _onAudioStreamController.close();
  }
